...
Live on three free tiers: **Neon** (PostgreSQL) · **Render** (API) · **Vercel** (web). The API runs
as a long-running Node process rather than serverless functions, because it holds a `pg` connection
pool and writes inside `SERIALIZABLE` transactions — the reasoning is in
[NOTES.md §11](./NOTES.md#11-docker--deployment).

Config lives in [`render.yaml`](./render.yaml) and [`apps/web/vercel.json`](./apps/web/vercel.json).
**Full runbook, including the three traps that cost real time: [docs/deployment.md](./docs/deployment.md).**
