...
```mermaid
stateDiagram-v2
    [*] --> PENDING: operator records a cleaning<br/>(a client-supplied status is ignored)
    PENDING --> PENDING: amend<br/>(audit entry, no reason required)
    PENDING --> VERIFIED: supervisor verifies<br/>(never their own cleaning)
    VERIFIED --> VERIFIED: amend<br/>(audit entry, reason REQUIRED)
    note right of VERIFIED
        No transition back to PENDING.
        A mistake is corrected by amending
        with a stated reason, which the
        trail keeps — not by reverting state.
    end note
```

Two rules worth calling out, both enforced in the API rather than only in the UI:
